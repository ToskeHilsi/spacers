...
class Game:
    ...
    def _handle_playing(self, event):
        ...
        if k == pygame.K_TAB:
            ...
            missile = self.player.try_missile()
            if missile:
    ...
    def update(self, dt):
        ...
        # Nebula stealth — mark anyone inside a dense nebula as hidden
        self.player.is_stealthed = False
        for dn in self.world.dense_nebulae:
            if dn.contains(self.player.x, self.player.y):
                self.player.is_stealthed = True
                break
        for enemy in self.world.enemies:
        ...
        for enemy in list(self.world.enemies):
            new_b = enemy.update(dt, self.player)
        ...
        # Check boss killed — expand zone
        current_boss_ids = set(id(b) for b in self.world.bosses)
        killed_bosses = prev_bosses - current_boss_ids
        # Also handle if boss was just marked dead but not removed yet
        for boss in list(self.world.bosses):
            if not boss.alive:
                killed_bosses.add(id(boss))
        if killed_bosses:
            new_zone = self.world.on_boss_killed()
    ...
    def _give_xp(self, amount, x, y):
        ...
        self.hud.add_message(f"+{amount} DATA", 1.0, (0, 200, 255))
