...
class Player:
    ...
    def try_missile(self):
        ...
        dx, dy = angle_to_vec(self.angle)
        return Missile(self.x, self.y, dx, dy, self.damage * 2.5, self.sprite_mgr)
...
class Asteroid:
    def __init__(self, x, y, radius):
        self.x, self.y = float(x), float(y)
        self.radius    = radius
        self._angle    = random.uniform(0, 360)   # slow drift rotation for visuals
    ...
    def draw(self, surf, cam_x, cam_y):
        ...
        if sx < -r or sx > sw+r or sy < -r or sy > sh+r:
            return
        # Draw as a jagged rock polygon
        pts = []
        sides = 10
        for i in range(sides):
            a   = math.radians(self._angle + 360 * i / sides)
            jag = r * random.uniform(0.75, 1.0)  # deterministic per-frame is fine visually
...
class Enemy:
    ...
    def __init__(self, x, y, sprite_key, hp, sprite_mgr):
        ...
        self.is_hidden      = False   # inside dense nebula
